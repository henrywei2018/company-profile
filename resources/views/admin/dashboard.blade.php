...
<x-layouts.admin :title="'Admin Dashboard'" :enableCharts="true" 
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white border rounded-xl p-6">
            ...
            <p class="text-2xl font-bold text-gray-900" id="users-count"></p>
            <p class="text-xs text-gray-600 mt-1">Active last 7 days: <span id="active-users"></span></p>
            <p class="text-xs text-gray-600">New this month: <span id="new-users"></span></p>
        </div>
        <!-- Projects -->
        <div class="bg-white border rounded-xl p-6">
            <h3 class="text-sm font-medium text-gray-500 mb-1">Total Projects</h3>
            <p class="text-2xl font-bold text-gray-900" id="projects-count"></p>
            <p class="text-xs text-gray-600 mt-1">Active: <span id="active-projects"></span></p>
            <p class="text-xs text-gray-600">New this month: <span id="new-projects"></span></p>
        </div>
        <!-- Quotations -->
        <div class="bg-white border rounded-xl p-6">
            <h3 class="text-sm font-medium text-gray-500 mb-1">Quotations</h3>
            <p class="text-2xl font-bold text-gray-900" id="quotations-count"></p>
            <p class="text-xs text-gray-600 mt-1">Pending: <span id="pending-quotations"></span></p>
            <p class="text-xs text-gray-600">New this month: <span id="new-quotations"></span></p>
        </div>
        <!-- Messages -->
        <div class="bg-white border rounded-xl p-6">
            <h3 class="text-sm font-medium text-gray-500 mb-1">Messages</h3>
            <p class="text-2xl font-bold text-gray-900" id="messages-count"></p>
            <p class="text-xs text-gray-600 mt-1">Unread: <span id="unread-messages"></span></p>
        </div>
        <!-- Testimonials, Services, Certifications, Chat Sessions -->
        <div class="bg-white border rounded-xl p-6">
            <h3 class="text-sm font-medium text-gray-500 mb-1">Testimonials</h3>
            <p class="text-2xl font-bold text-gray-900" id="testimonials-count"></p>
        </div>
        <div class="bg-white border rounded-xl p-6">
            <h3 class="text-sm font-medium text-gray-500 mb-1">Services</h3>
            <p class="text-2xl font-bold text-gray-900" id="services-count"></p>
        </div>
        <div class="bg-white border rounded-xl p-6">
            <h3 class="text-sm font-medium text-gray-500 mb-1">Certifications</h3>
            <p class="text-2xl font-bold text-gray-900" id="certifications-count"></p>
        </div>
        <div class="bg-white border rounded-xl p-6">
            <h3 class="text-sm font-medium text-gray-500 mb-1">Chat Sessions</h3>
            <p class="text-2xl font-bold text-gray-900" id="chat-sessions-count"></p>
        </div>
    </div>

    <!-- Breakdown and Growth Metrics -->
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <!-- Project Status Breakdown -->
        <div class="bg-white border rounded-xl p-6">
            <h3 class="text-sm font-semibold text-gray-900 mb-3">Project Status Breakdown</h3>
            <ul id="project-status-breakdown" class="space-y-1"></ul>
        </div>
        <!-- Quotation Status Breakdown -->
        <div class="bg-white border rounded-xl p-6">
            <h3 class="text-sm font-semibold text-gray-900 mb-3">Quotation Status Breakdown</h3>
            <ul id="quotation-status-breakdown" class="space-y-1"></ul>
        </div>
    </div>
    <!-- Timestamp -->
    <div class="text-xs text-right text-gray-400 mb-2">
        <span>Last updated: <span id="stats-timestamp"></span></span>
        <span id="stats-cached"></span>
    </div>
</div>
...
function updateStats(stats, timestamp, cached) {
    document.getElementById('users-count').innerText = stats.users_count ?? 0;
    document.getElementById('active-users').innerText = stats.active_users ?? 0;
    document.getElementById('new-users').innerText = stats.new_users_this_month ?? 0;

    document.getElementById('projects-count').innerText = stats.projects_count ?? 0;
    document.getElementById('active-projects').innerText = stats.active_projects ?? 0;
    document.getElementById('new-projects').innerText = stats.new_projects_this_month ?? 0;

    document.getElementById('quotations-count').innerText = stats.quotations_count ?? 0;
    document.getElementById('pending-quotations').innerText = stats.pending_quotations ?? 0;
    document.getElementById('new-quotations').innerText = stats.new_quotations_this_month ?? 0;

    document.getElementById('messages-count').innerText = stats.messages_count ?? 0;
    document.getElementById('unread-messages').innerText = stats.unread_messages ?? 0;

    document.getElementById('testimonials-count').innerText = stats.testimonials_count ?? 0;
    document.getElementById('services-count').innerText = stats.services_count ?? 0;
    document.getElementById('certifications-count').innerText = stats.certifications_count ?? 0;
    document.getElementById('chat-sessions-count').innerText = stats.chat_sessions_count ?? 0;

    // Project Status Breakdown
    let projectStatusList = document.getElementById('project-status-breakdown');
    projectStatusList.innerHTML = '';
    for (const [status, count] of Object.entries(stats.project_status_breakdown ?? {})) {
        let li = document.createElement('li');
        li.innerHTML = `<span class="font-semibold capitalize">${status.replace(/_/g, ' ')}:</span> ${count}`;
        projectStatusList.appendChild(li);
    }

    // Quotation Status Breakdown
    let quotationStatusList = document.getElementById('quotation-status-breakdown');
    quotationStatusList.innerHTML = '';
    for (const [status, count] of Object.entries(stats.quotation_status_breakdown ?? {})) {
        let li = document.createElement('li');
        li.innerHTML = `<span class="font-semibold capitalize">${status.replace(/_/g, ' ')}:</span> ${count}`;
        quotationStatusList.appendChild(li);
    }

    // Timestamp
    document.getElementById('stats-timestamp').innerText = new Date(timestamp).toLocaleString();
    document.getElementById('stats-cached').innerText = cached ? ' (cached)' : '';
}
    ...
    axios.get('{{ route('admin.dashboard.stats') }}')
        .then(function(response) {
            if (response.data && response.data.success) {
                updateStats(response.data.data, response.data.timestamp, response.data.cached);
            }
        })
...
document.addEventListener('DOMContentLoaded', function() {
    fetchStats();
    // Optionally auto-refresh every 2 minutes:
