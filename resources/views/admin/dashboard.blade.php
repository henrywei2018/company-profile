...
<x-layouts.admin :title="'Dashboard'" :enableCharts="true">
    ...
    <div class="mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <!-- Projects Stat -->
            <x-admin.stat-card title="Total Projects" value="{{ $totalProjects }}"
            ...
            <!-- Clients Stat -->
            <x-admin.stat-card title="Active Clients" value="{{ $activeClients }}"
                icon="<path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' />"
                iconColor="text-blue-500" iconBg="bg-blue-100" :change="$clientsChange"
                href="{{ route('admin.users.index', ['role' => 'client']) }}" />

            <!-- Messages Stat -->
            <x-admin.stat-card title="Unread Messages" value="{{ $unreadMessages }}"
                icon="<path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' />"
                iconColor="text-red-500" iconBg="bg-red-100"
                href="{{ route('admin.messages.index', ['read' => 'unread']) }}" />

            <!-- Quotations Stat -->
            <x-admin.stat-card title="Pending Quotations" value="{{ $pendingQuotations }}"
    ...
    <div class="mb-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <x-recent-activity title="Recent Messages" viewAllRoute="{{ route('admin.messages.index') }}">
            @forelse($recentMessages as $message)
                <x-activity-item route="{{ route('admin.messages.show', $message) }}"
                    icon="<path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' />"
        ...
        <x-recent-activity title="Recent Quotations" viewAllRoute="{{ route('admin.quotations.index') }}">
            @forelse($recentQuotations as $quotation)
                <x-activity-item route="{{ route('admin.quotations.show', $quotation) }}"
