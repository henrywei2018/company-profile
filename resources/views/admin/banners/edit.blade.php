                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="link_type" id="link_type" onchange="updateLinkPlaceholder()"
                                ...
                                <option value="auto" {{ old('link_type', $banner->link_type ?? 'auto') === 'auto' ? 'selected' : '' }}>Auto-detect</option>
                                <option value="internal" {{ old('link_type', $banner->link_type) === 'internal' ? 'selected' : '' }}>Internal Link</option>
                                <option value="external" {{ old('link_type', $banner->link_type) === 'external' ? 'selected' : '' }}>External Link</option>
                                <option value="route" {{ old('link_type', $banner->link_type) === 'route' ? 'selected' : '' }}>Laravel Route</option>
                                <option value="email" {{ old('link_type', $banner->link_type) === 'email' ? 'selected' : '' }}>Email Address</option>
                                <option value="phone" {{ old('link_type', $banner->link_type) === 'phone' ? 'selected' : '' }}>Phone Number</option>
                                <option value="anchor" {{ old('link_type', $banner->link_type) === 'anchor' ? 'selected' : '' }}>Anchor Link</option>
                            </select>
                        </div>
                ...
                <x-admin.card>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                @if($banner->hasDesktopImage())
                                    <div class="relative group">
                                        <div class="aspect-w-16 aspect-h-9 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700">
                                            <img src="{{ $banner->imageUrl }}" alt="Desktop Image" class="w-full h-full object-cover">
                                ...
                                @if($banner->hasMobileImage())
                                    <div class="relative group">
                                        <div class="aspect-w-9 aspect-h-16 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700">
                                            <img src="{{ $banner->mobileImageUrl }}" alt="Mobile Image" class="w-full h-full object-cover">
                                            <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 transition-all duration-200"></div>
                                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <x-universal-file-uploader
                            id="banner-images-uploader"
                            name="banner_images"
                            :multiple="true"
                            :maxFiles="2"
                            maxFileSize="5MB"
                            :acceptedFileTypes="['image/jpeg', 'image/png', 'image/jpg', 'image/gif', 'image/webp']"
                            :uploadEndpoint="route('admin.banners.upload-image', $banner)"
                            :deleteEndpoint="route('admin.banners.delete-image', $banner)"
                            dropDescription="Drop new banner images here or click to browse"
