<x-layouts.admin>
    <x-slot name="title">Edit Payment Method</x-slot>
    ...
    <div class="container mx-auto px-6 py-8">
            ...
            <nav class="flex" aria-label="Breadcrumb">
                <ol role="list" class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            <i class="fas fa-chevron-right text-gray-300 mr-4"></i>
                            <span class="ml-4 text-sm font-medium text-gray-500">Edit {{ $paymentMethod->name }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 class="text-lg font-medium text-gray-900">Edit Payment Method</h1>
                            <p class="text-sm text-gray-500">Update the payment method details</p>
                        </div>
                    </div>
            ...
            <form action="{{ route('admin.payment-methods.update', $paymentMethod) }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div class="col-span-2 bank-details" style="display: none;">
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                            ...
                            <div>
                                ...
                                <input type="text" name="bank_code" id="bank_code" value="{{ old('bank_code', $paymentMethod->bank_code) }}" 
                                       class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" 
                    ...
                    <div class="col-span-2">
                        ...
                        <div class="mt-1 flex items-center space-x-4">
                            <img src="{{ $paymentMethod->logo_url }}" alt="{{ $paymentMethod->name }} logo" class="h-12 w-auto">
                            <span class="text-sm text-gray-500">Current logo will be replaced if you upload a new one</span>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="col-span-2">
                        <label for="logo" class="block text-sm font-medium text-gray-700">
                            {{ $paymentMethod->logo ? 'Update Logo (Optional)' : 'Upload Logo' }}
                        </label>
                                ...
                                <div id="logo-placeholder">
                                    ...
                                    <div class="text-sm text-gray-600">
                                        <label for="logo" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                            <span>{{ $paymentMethod->logo ? 'Change logo' : 'Upload a logo' }}</span>
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="instructions" id="instructions" rows="4" 
                                  class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" 
                                  placeholder="Provide step-by-step instructions for customers...">{{ old('instructions', $paymentMethod->instructions) }}</textarea>
                        @error('instructions')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
