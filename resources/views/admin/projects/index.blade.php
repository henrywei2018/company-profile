                        ...
                        <x-admin.table-cell>
                            <div class="flex items-center space-x-2">
                                ...
                                <form action="{{ route('admin.projects.toggle-featured', $project) }}" method="POST" class="inline">
                                    ...
                                    @method('PATCH')
                                    <button type="submit"
                                        class="inline-flex items-center justify-center rounded p-2 {{ $project->featured ? ' text-yellow-700 hover:text-yellow-200' : ' text-teal-700 hover:text-teal-200' }} transition focus:outline-none"
                                        title="{{ $project->featured ? 'Remove from featured' : 'Mark as featured' }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="{{ $project->featured ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                                        </svg>
                                    </button>
                                </form>
                            
                                <!-- Dropdown for Edit, Delete, View -->
                                <div class="relative inline-block text-left" x-data="{ open: false }">
                                    <button @click="open = !open"
                                        class="inline-flex items-center justify-center rounded p-2 text-gray-700 hover:text-gray-200 transition focus:outline-none"
                                        title="More actions">
                                    ...
                                    <div x-show="open" @click.away="open = false"
                                        ...
                                        <div class="py-1">
                                            ...
                                            <form action="{{ route('admin.projects.destroy', $project) }}" method="POST"
                                                onsubmit="return confirm('Are you sure you want to delete this project?')" class="inline w-full">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
