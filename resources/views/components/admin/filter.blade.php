...
<div {{ $attributes->merge(['class' => 'bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-xl shadow-sm mb-6']) }} 
    ...
    <div class="px-4 py-2 border-gray-200 dark:border-neutral-700 flex flex-wrap justify-between items-center gap-3">
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
        <form action="{{ $action }}" method="{{ $method }}" class="p-4 space-y-2">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {{ $slot }}
            </div>
            
            <div class="flex items-center justify-end gap-3 pt-2 border-t border-gray-200 dark:border-neutral-700">
                @if($resetRoute)
                ...
                @endif
                <button type="submit" class="inline-flex justify-center px-2 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-neutral-800">
                    Apply Filters
                </button>
            </div>
        </form>
