...
<div id="{{ $id }}" class="hs-overlay hidden w-full h-full fixed top-0 left-0 z-[60] overflow-x-hidden overflow-y-auto" {{ $static ? 'data-hs-overlay-keyboard="false" data-hs-overlay-backdrop="static"' : '' }}>
    <div class="hs-overlay-open:opacity-100 hs-overlay-open:duration-500 opacity-0 transition-all sm:max-w-lg w-full m-3 sm:mx-auto {{ $modalSize }}">
        <div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-neutral-800 dark:border-neutral-700 dark:shadow-neutral-700/[.7]">
