...
<x-layouts.admin title="New Message" :unreadMessages="0" :pendingQuotations="0">
    <!-- Header -->
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <x-admin.card>
                ...
                <form action="{{ route('client.messages.store') }}" method="POST" enctype="multipart/form-data" id="message-form">
                    @csrf
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                >
                                    @foreach($messageTypes as $value => $label)
                                        <option value="{{ $value }}" {{ old('type', $prefillData['type'] ?? 'general') === $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    class="block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                                >
                                    <option value="low" {{ old('priority', $prefillData['priority'] ?? 'normal') === 'low' ? 'selected' : '' }}>Low</option>
                                    <option value="normal" {{ old('priority', $prefillData['priority'] ?? 'normal') === 'normal' ? 'selected' : '' }}>Normal</option>
                                    <option value="high" {{ old('priority', $prefillData['priority'] ?? 'normal') === 'high' ? 'selected' : '' }}>High</option>
                                    <option value="urgent" {{ old('priority', $prefillData['priority'] ?? 'normal') === 'urgent' ? 'selected' : '' }}>Urgent</option>
                                </select>
                                @error('priority')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                        ...
                        <div>
                            ...
                            >
                                ...
                                @foreach($projects as $project)
                                    <option value="{{ $project->id }}" {{ old('project_id', $prefillData['project_id'] ?? '') == $project->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="flex justify-between mt-1">
                                ...
                                @else
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Be as detailed as possible to help us assist you better</p>
                                @enderror
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <x-admin.card>
                ...
                <div class="space-y-3">
                    ...
                    <div class="text-sm">
                        <a href="{{ route('client.messages.show', $recentMessage['id']) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium">
...
function displaySelectedFiles(input) {
    ...
    if (input.files.length > 0) {
        ...
        Array.from(input.files).forEach((file, index) => {
            ...
            fileItem.className = 'flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-md text-sm';
            ...
            removeButton.className = 'text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 p-1';
