...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withSchedule(function (Schedule $schedule) {
        ...
        // Chat queue processing every minute
        $schedule->command('chat:process-queue --auto-assign')
            ->everyMinute()
            ->description('Auto-assign waiting chat sessions')
            ->appendOutputTo(storage_path('logs/chat-queue.log'));

        // Priority boost every 5 minutes
        $schedule->command('chat:process-queue --priority-boost')
            ->everyFiveMinutes()
            ->description('Boost priority for long-waiting sessions')
            ->appendOutputTo(storage_path('logs/chat-queue.log'));

        // Timeout check every 10 minutes
        $schedule->command('chat:process-queue --timeout-check')
            ->everyTenMinutes()
            ->description('Handle chat session timeouts')
            ->appendOutputTo(storage_path('logs/chat-queue.log'));

        // Queue health monitoring every hour
        $schedule->command('chat:queue-status')
            ->hourly()
            ->description('Log queue status for monitoring')
            ->appendOutputTo(storage_path('logs/chat-monitoring.log'));

        // Clean up old sessions daily
        $schedule->call(function () {
            app(\App\Services\ChatService::class)->archiveOldSessions(30);
        })->daily()->description('Archive old chat sessions');
    })
    ->withProviders([
